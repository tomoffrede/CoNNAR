...
folderData <- paste0("C:/Users/offredet/Documents/1HU/CoNNAR/data/try/")
# folderTurn <- paste0(here::here(), "/data/final/")

filesTG <- list.files(folderData, "TextGrid")
filesTG <- filesTG[grepl("free", filesTG)]
filesTXT <- list.files(folderData, "\\.txt")
# filesT <- list.files(folderTurn, "\\.TextGrid")

ff <- data.frame(filesTG, filesTXT) %>%
  mutate(worked = ifelse(gsub(".TextGrid", "", filesTG) == gsub(".txt", "", filesTXT), # to make sure the files are mathing
                         "worked!", "NO!!!!!"))
table(ff$worked)
```
...
names(int) <- c("file", "dyad", "speaker", "confederate", "turn", "intervalIPU", "label", "ipuOnset", "ipuOffset", "ipuDur", "deleteDur", "vowels", "f0mean", "f0med", "f0max", "f0sd")
      ...
      for(r in 1:nrow(txt)){
        if(abs(txt$onset[r] - int$turnOnset[i]) < 0.01){
          if(abs(txt$offset[r] - int$turnOffset[i]) < 0.01){
            ...
            int$f0max[i] <- as.numeric(txt$f0max)[r]
...
dat <- dat %>% 
  group_by(file) %>% 
  mutate_at(c("turn", "f0mean", "f0med", "f0max", "f0sd"), as.numeric) %>% 
  mutate(prevTurn = ifelse(speaker == firstSp, turn-1, turn)) %>%
  mutate(prevTurn = ifelse(speaker == firstSp & turn == 1, NA, prevTurn),
         prevAR = NA, prevf0mean = NA, prevf0med = NA, prevf0max = NA, prevf0sd = NA) %>%
  ungroup()
...
for(i in 1:nrow(dat)){
  ...
  prevf0mean <- as.numeric(dat$f0mean[dat$dyad == dat$dyad[i] &
                         dat$speaker != dat$speaker[i] &
                         dat$turn == dat$prevTurn[i]])
  prevf0med <- as.numeric(dat$f0med[dat$dyad == dat$dyad[i] &
                         dat$speaker != dat$speaker[i] &
                         dat$turn == dat$prevTurn[i]])
  prevf0max <- as.numeric(dat$f0max[dat$dyad == dat$dyad[i] &
                         dat$speaker != dat$speaker[i] &
                         dat$turn == dat$prevTurn[i]])
  prevf0sd <- as.numeric(dat$f0sd[dat$dyad == dat$dyad[i] &
                         dat$speaker != dat$speaker[i] &
                         dat$turn == dat$prevTurn[i]])
  ...
  if(!purrr::is_empty(prevf0max)){
    if(!any(is.na(prevf0max))){
      dat$prevf0max[i] <- as.numeric(prevf0max)
    }
  }
...
dat <- dat %>% 
  mutate(artRateDiff = abs(turnAR - prevAR),
         f0meanDiff = abs(f0mean - prevf0mean),

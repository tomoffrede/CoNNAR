...
for(i in 1:nrow(ff)){
  ...
  int <- int %>% 
    mutate(distanceOnsets = onset - previousOnset,
           distanceOffsets = offset - previousOffset) %>% 
    mutate(turn = case_when(
      onset > previousOnset & offset < previousOffset ~ as.numeric(NA),
      abs(distanceOnsets) <= 0.12 & duration < previousDur ~ as.numeric(NA),
      abs(distanceOffsets) <= 0.12 & duration < previousDur ~ as.numeric(NA),
...
###### put the following chunk somewhere in the loop above to add the last IPU from the last speaker on each row (and then save the timing info of each IPU):
